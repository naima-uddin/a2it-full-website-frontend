...
import { fetchEmployees, fetchDepartments, exportReport, testApiConnection } from "@/app/hrm/lib/report";
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (!connectionTest.success) {
        ...
        toast.error(
          <div>
            <div className="font-semibold">âš ï¸ Backend Connection Failed</div>
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (result instanceof Blob) {
        ...
        toast.success(
          <div>
            <div className="font-semibold">âœ… Report Generated Successfully!</div>
        ...
        if (result.success) {
          toast.success(
            <div>
              <div className="font-semibold">ðŸ“Š Report Data Ready</div>
      ...
      toast.error(
        <div>
          <div className="font-semibold">âŒ Export Failed</div>
          <div className="text-sm">{error.message || 'Please check your connection and try again'}</div>
          <div className="text-xs mt-1 opacity-75">
            Report Type: {getReportTypeName(reportType)} â€¢ Format: {exportType.toUpperCase()}
          </div>
        </div>,
        {
          duration: 7000,
                ...
                <div>
                  ...
                  <p className={`text-lg font-bold ${
                    ...
                    {apiStatus.loading ? 'Checking connection...' :
                     apiStatus.connected ? 'Connected âœ“' :
                     'Disconnected âœ—'}
                  </p>
                </div>
                      ...
                      return history.slice(0, 5).map((report) => (
                        <tr key={report.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                          ...
                          <td className="py-3 px-4 text-sm text-gray-700">
                            {new Date(report.timestamp).toLocaleDateString('en-BD', { timeZone: 'Asia/Dhaka', day: '2-digit', month: 'short', year: 'numeric' })} {new Date(report.timestamp).toLocaleTimeString('en-BD', { timeZone: 'Asia/Dhaka', hour: '2-digit', minute: '2-digit', hour12: true })}
                          </td>
